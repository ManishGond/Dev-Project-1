table 50104 "Approval Entry Table"
{
    Caption = 'Approval Entry Table';
    ReplicateData = true;
    ...
    fields
    {
        ...
        {
            CalcFormula = Count("Approval Entry Table" WHERE("Record ID to Approve" = FIELD("Record ID to Approve"),
    ...
    local procedure OnAfterGetCustVendorDetails(var ApprovalEntry: Record "Approval Entry Table"; var CustVendorNo: Code[20]; var CustVendorName: Text[100])
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeMarkAllWhereUserisApproverOrSender(var ApprovalEntry: Record "Approval Entry Table"; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeShowRecord(var ApprovalEntry: Record "Approval Entry Table"; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeRecordCaption(var ApprovalEntry: Record "Approval Entry Table"; var Result: Text; var IsHandled: Boolean)
    begin
    end;
}
